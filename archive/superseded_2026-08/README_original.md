...
3. [Live Portfolio Demo](#live-portfolio-demo)
...
A high-fidelity, interactive simulation of the MAOR-Equity pipeline is available in `demo.html`. This demo showcases:
- **Distributed Agent Orchestration**: Real-time visualization of Node A and Node B communication.
- **Logic Inspector**: Live code snippets showing the underlying Python logic for each agent.
- **3-D Sentiment Matrix**: Visualization of the multi-dimensional FinBERT output.
- **Interactive Ticker Analysis**: Test the pipeline on AAPL, MSFT, and TSLA.

To view locally: Open `demo.html` in any modern web browser.
To host on GitHub: See `deploy_demo.ps1` for instructions.
